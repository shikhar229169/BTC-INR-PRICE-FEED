 ...
 */
contract BtcToInrPriceFeed is FunctionsClient, ConfirmedOwner, AutomationCompatibleInterface {
  ...
  address public constant priceFeedsBtcToUsd = 0x1b44F3514812d835EB1BDB0acB33d3fA3351Ee43;
  ...
  function fulfillRequest(bytes32 requestId, bytes memory response, bytes memory err) internal override {
    (,int256 btcToUsd,,,) = AggregatorV3Interface(priceFeedsBtcToUsd).latestRoundData();

    uint256 usdToInr = abi.decode(response, (uint256));

    priceAns = (uint256(btcToUsd) * usdToInr) / 1e8;
